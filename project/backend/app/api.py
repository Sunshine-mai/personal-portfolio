...
from app.models import AdminUser, AuditEvent, Project, Revision
from app.schemas import AuditEventOut, LoginRequest, ProjectCreate, ProjectOut, RevisionOut
...
def audit(db: Session, actor: str, action: str, resource_type: str, resource_id: int | None = None, detail: str = "") -> None:
    db.add(AuditEvent(actor=actor, action=action, resource_type=resource_type, resource_id=str(resource_id) if resource_id is not None else None, detail=detail))
...
@admin.post("/projects")
def create_project(payload: ProjectCreate, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    ...
    db.add(revision); audit(db, actor, "PROJECT_CREATED", "project", p.id); db.commit(); db.refresh(p)
    return {"code": 200, "message": "success", "data": project_data(p, revision.id)}

@admin.post("/projects/{project_id}/submit-review")
def submit_review(project_id: int, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    ...
    revision.status = "IN_REVIEW"; p.status = "IN_REVIEW"; audit(db, actor, "REVISION_SUBMITTED", "revision", revision.id); db.commit()
    return {"code": 200, "message": "success", "data": {"revision_id": revision.id, "status": revision.status}}

@admin.put("/projects/{project_id}")
def update_project(project_id: int, payload: ProjectCreate, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    p = db.get(Project, project_id)
    if not p: raise HTTPException(404, "RESOURCE_NOT_FOUND")
    ...
    p.status = "DRAFT"; p.published = False; db.add(revision); db.flush(); audit(db, actor, "PROJECT_UPDATED", "project", p.id); db.commit(); db.refresh(p)
    return {"code": 200, "message": "success", "data": project_data(p, revision.id)}

@admin.post("/revisions/{revision_id}/reject")
def reject(revision_id: int, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    ...
    revision.status = "CHANGES_REQUESTED"; revision.project.status = "DRAFT"; audit(db, actor, "REVISION_REJECTED", "revision", revision.id); db.commit()
    return {"code": 200, "message": "success", "data": {"revision_id": revision.id, "status": revision.status}}

@admin.post("/projects/{project_id}/unpublish")
def unpublish(project_id: int, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    p = db.get(Project, project_id)
    if not p: raise HTTPException(404, "RESOURCE_NOT_FOUND")
    p.published = False; p.status = "UNPUBLISHED"; audit(db, actor, "PROJECT_UNPUBLISHED", "project", p.id); db.commit()
    return {"code": 200, "message": "success", "data": {"project_id": p.id, "status": p.status}}

@admin.post("/revisions/{revision_id}/approve")
def approve(revision_id: int, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    revision = db.get(Revision, revision_id)
    if not revision: raise HTTPException(404, "RESOURCE_NOT_FOUND")
    if revision.status != "IN_REVIEW": raise HTTPException(409, "INVALID_STATE_TRANSITION")
    revision.status = "APPROVED"; revision.project.status = "APPROVED"; audit(db, actor, "REVISION_APPROVED", "revision", revision.id); db.commit()
...
def publish(revision_id: int, actor: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    revision = db.get(Revision, revision_id)
    if not revision: raise HTTPException(404, "RESOURCE_NOT_FOUND")
    if revision.status != "APPROVED": raise HTTPException(409, "REVIEW_REQUIRED")
    revision.status = "PUBLISHED"; revision.project.published = True; revision.project.status = "PUBLISHED"; audit(db, actor, "REVISION_PUBLISHED", "revision", revision.id); db.commit()
...
def list_audit_events(_: str = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    events = db.scalars(select(AuditEvent).order_by(AuditEvent.created_at.desc(), AuditEvent.id.desc()).limit(100)).all()
    return {"code": 200, "message": "success", "data": [AuditEventOut(id=e.id, actor=e.actor, action=e.action, resource_type=e.resource_type, resource_id=e.resource_id, detail=e.detail, created_at=e.created_at.isoformat()).model_dump() for e in events]}
