...
// 剪辑作品：首页只放最新 6 条，全部作品在 /edit-works。
// playingBvid 由首页持有再下发，这样点开另一条时上一条的 iframe 会被卸载——
// 同时只播一个。封面是否失效的兜底状态在卡片组件内部，各卡独立。
const playingBvid = ref(null)
const latestWorks = computed(() => editWorks.slice(0, 6))
  ...
  <div>
    ...
    <section class="content-section edit-section" id="edit-works">
      <div class="shell">
        ...
        <!-- 首页只放最新 6 条。全部 19 条会让这一节变成 7 行，把工程方法与关于我
             压到很下面——首页的职责是"让人快速判断你做什么"，作品仓库放在单独一页。 -->
        <div class="edit-grid">
          <EditWorkCard
        ...
        <RouterLink class="edit-more" to="/edit-works">
          查看全部 {{ editWorks.length }} 个作品 <span aria-hidden="true">→</span>
        </RouterLink>
      </div>
    </section>
