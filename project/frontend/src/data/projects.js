...
export const fallbackProjects = [
  { id: 1, slug: 'ai-translator', title: '词流 LexiFlow', repoName: 'ai-translator', summary: '面向英语学习者的无广告翻译与生词学习工具，把一次翻译延展为可持续的学习闭环。', background: '翻译、收藏、复习和统计共同组成学习闭环。', outcome: '已实现核心闭环，等待补齐正式发布证据。', role: '产品设计、全栈开发、部署与验证规划', projectType: '语言学习 / 翻译工具', project_type: 'INDEPENDENT', status: '已实现 · 待补正式发布证据', tags: ['翻译', '生词复习', 'FastAPI'], image: '/assets/v5/translator-home.png', visualKind: 'screenshot', evidence: '3 张已核验截图 · 引擎降级提示的证据待补', stack: [{ layer: '前端', items: ['Vue 3', 'Element Plus', 'Pinia', 'Tailwind CSS'], note: '翻译、词典、生词本与复习界面' }, { layer: '后端', items: ['Python 3.11+', 'FastAPI', 'SQLAlchemy', 'Alembic'], note: '接口、游客配额与学习闭环编排' }, { layer: '数据', items: ['SQLite / PostgreSQL', 'Redis'], note: '开发用 SQLite 零配置，生产切 PostgreSQL；Redis 不可用时降级' }, { layer: '运行', items: ['Docker Compose', 'Nginx'], note: '前端多阶段构建后由 Nginx 托管' }], structure: { root: 'ai-translator/', note: '脱敏摘要 · 省略配置文件、环境变量与私有目录', nodes: [{ depth: 0, name: 'backend/', note: 'FastAPI 服务' }, { depth: 1, name: 'app/routers/', note: '接口路由' }, { depth: 1, name: 'app/services/', note: '翻译引擎、词典与学习闭环' }, { depth: 1, name: 'app/models/', note: '数据表模型' }, { depth: 1, name: 'app/schemas/', note: '请求与响应校验' }, { depth: 1, name: 'app/middleware/', note: '认证与限流' }, { depth: 0, name: 'frontend/', note: 'Vue 3 界面' }, { depth: 1, name: 'src/views/', note: '首页、翻译、生词本、复习' }, { depth: 1, name: 'src/components/', note: '通用组件' }, { depth: 1, name: 'src/stores/', note: '状态管理' }, { depth: 1, name: 'src/api/', note: '接口封装' }, { depth: 0, name: 'nginx/', note: '反向代理与静态托管' }, { depth: 0, name: 'docker-compose.yml', note: '开发编排' }, { depth: 0, name: 'docker-compose.prod.yml', note: '生产编排' }] }, gallery: [{ src: '/assets/v5/translator-home.png', step: 'STEP 01 / 入口', caption: '首页「每日一词」。把工具的入口前置成每天可以回访的学习动作，而不是一次性的翻译框。' }, { src: '/assets/v5/lexiflow-translate-result.png', step: 'STEP 02 / 核心链路', caption: '真实翻译结果。输入一句英文，模型返回中文译文，下方列出可继续查词的高频词。截图取自本地实跑，不是设计稿。' }, { src: '/assets/v5/lexiflow-quota-limit.png', step: 'STEP 03 / 边界处理', caption: '匿名游客每日 3 次免费翻译，用完后不静默失败，而是引导登录。配额按匿名标识独立计数，登录用户不受限。' }] },
  { id: 2, slug: 'ai-second-brain', title: '问册 Folio', repoName: 'ai-second-brain', summary: '把文档整理、切片检索与问答连成一条链路，回答只依据上传的资料，并标注来源。', background: '把文档上传、切片、检索和流式对话连接起来。', outcome: '已完成独立项目闭环，正在整理公开版本证据。', role: '产品建模、Java 全栈开发、测试与部署', projectType: '知识库 / 检索问答', project_type: 'SOLO', status: '已实现 · 发布前复核', tags: ['文档检索', '引用溯源', 'Spring Boot'], image: '/assets/v5/folio-cover-login.png', visualKind: 'screenshot', evidence: '3 张已核验截图 · 均为独立演示账号采集，不含真实资料', stack: [{ layer: '前端', items: ['Vue 3', 'Element Plus', 'Pinia', 'Tailwind CSS'], note: '登录、文档管理与检索对话界面' }, { layer: '后端', items: ['Java 17', 'Spring Boot 3', 'Sa-Token', 'MyBatis-Plus', 'SpringDoc'], note: '接口、认证与业务编排' }, { layer: '检索与模型', items: ['LangChain4j', 'Apache Tika', 'BGE-Small-ZH'], note: '文档解析、递归切片与本地向量化检索' }, { layer: '数据', items: ['MySQL'], note: '文档、切片、会话与用户' }, { layer: '运行', items: ['Docker', 'Nginx'], note: '容器编排与反向代理' }], structure: { root: 'ai-second-brain/', note: '脱敏摘要 · 省略配置文件、环境变量与私有目录', nodes: [{ depth: 0, name: 'backend/', note: 'Spring Boot 服务' }, { depth: 1, name: 'src/main/java/com/aibrain/', note: '领域代码根' }, { depth: 2, name: 'controller/', note: '接口层' }, { depth: 2, name: 'service/', note: '文档解析、切片、检索与对话编排' }, { depth: 2, name: 'mapper/', note: '数据访问' }, { depth: 2, name: 'entity/', note: '数据表映射' }, { depth: 2, name: 'dto/', note: '请求与响应对象' }, { depth: 2, name: 'config/', note: '认证、跨域、异步与模型配置' }, { depth: 0, name: 'frontend/', note: 'Vue 3 界面' }, { depth: 1, name: 'src/pages/', note: '登录、文档管理、对话' }, { depth: 1, name: 'src/components/', note: '会话、文档卡片等组件' }, { depth: 1, name: 'src/composables/', note: '对话与文档的可复用逻辑' }, { depth: 1, name: 'src/stores/', note: '登录态与会话状态' }, { depth: 1, name: 'src/api/', note: '接口封装' }, { depth: 0, name: 'deploy/', note: 'nginx 与生产配置' }, { depth: 0, name: 'docker-compose.yml', note: 'MySQL + 服务 + Nginx 编排' }] }, gallery: [{ src: '/assets/v5/secondbrain-login.png', step: 'STEP 01 / 身份入口', caption: '登录页。左侧说明产品定位，右侧为登录表单。系统区分管理员与普通用户，文档、会话与模型配置按账号隔离。' }, { src: '/assets/v5/brain-documents.png', step: 'STEP 02 / 文档管理', caption: '文档上传与处理状态。上传后经过解析、递归切片和本地 BGE 向量化，页面上显示每个文档的切片数量。截图为演示账号，未包含真实资料。' }, { src: '/assets/v5/brain-chat-cited.png', step: 'STEP 03 / 核心链路', caption: '知识库对话。回答基于检索到的片段生成，并给出引用片段与相似度分数；这份材料只覆盖了问题的一部分，回答就明确说明缺失，而不是编造补全。' }] },
  { id: 3, slug: 'zhiheng-teaching', title: '高中个性化教学平台 · 知衡', summary: '围绕题库、考试、掌握度、错题和练习构建三角色教学分析闭环。', background: '用清晰的数据流连接管理员、老师和学生的日常工作。', outcome: '补齐学生端与管理员端页面、知识点建表与知识图谱，然后处理部署。当前为单机演示数据，未压测、未部署。', role: '产品定义、信息架构、Java 后端与 Vue 前端开发、测试与验收', projectType: '教育产品 / 全栈应用', project_type: 'INDEPENDENT', status: '教师端考试闭环已交付 · 学生端后端完成、前端待补', tags: ['Spring Boot', 'Vue 3', '三角色'], image: '/assets/v5/zhiheng-dashboard.png', visualKind: 'screenshot', evidence: '真实前后端与数据库 · 8 次版本化迁移 · 51 项后端测试 · 考试分析页存在接口缺陷待修', stack: [{ layer: '前端', items: ['Vue 3', 'TypeScript', 'Element Plus', 'Pinia'], note: '三角色工作台、题库与逐题改卷界面' }, { layer: '后端', items: ['Java 17', 'Spring Boot 3', 'MyBatis-Plus', 'Sa-Token', 'EasyExcel'], note: '接口、角色权限、考试流程与题库批量导入' }, { layer: '数据', items: ['MySQL', 'Flyway', 'H2'], note: '8 个版本化迁移；测试用内存库隔离。尚未部署，故没有运行层' }], gallery: [{ src: '/assets/v5/zhiheng-dashboard.png', step: 'STEP 01 / 教师工作台', caption: '教师工作台。班级平均掌握度、待批改考试、需关注学生与可用题目数都由最近一次真实考试的结果聚合得出，数据来自后端接口而非写死的演示值。' }, { src: '/assets/v5/zhiheng-questions.png', step: 'STEP 02 / 题库管理', caption: '题库管理：支持单选、多选、判断、填空、解答五种题型，可维护知识点、难度与分值，也支持按模板 Excel 批量导入。' }, { src: '/assets/v5/zhiheng-grading.png', step: 'STEP 03 / 逐题改卷', caption: '逐题改卷。按学生逐题录入得分，前端限制分数不得超过该题满分，左侧实时显示每位学生的录入进度。这是线下考试闭环里最关键的一步。' }] },
  { id: 4, slug: 'university-news', title: '大学新闻网', summary: 'PC 管理端、移动 Web 与微信小程序共享后端 API 的校园新闻系统。', background: '合作项目，覆盖内容管理、多端展示和文件存储。', outcome: '已按团队口径说明个人贡献边界，公开范围限于读者端页面截图。', role: '合作项目，双方共同参与：后端以搭档为主，前端 UI 以我为主；搭档为项目发起者，整体贡献多于我。具体模块划分未逐项记录。', projectType: '内容平台 / 多端体验', project_type: 'COLLABORATIVE', status: '已授权展示 · 贡献边界已说明', tags: ['合作', '多端', '内容管理'], image: '/assets/v5/nanyang-1.png', visualKind: 'screenshot', evidence: '3 张读者端页面截图（由项目成员提供） · 图注已按截图实况核对 · 后台管理端未提供截图', stack: [{ layer: 'PC 前端', items: ['Vue 3', 'Tailwind CSS', 'HeadlessUI', 'wangEditor', 'Swiper'], note: '读者端展示与管理端界面' }, { layer: '移动与小程序', items: ['Vue 3', 'Vant', 'Vant Weapp'], note: '移动 Web 与微信小程序两套端，共用同一套后端接口' }, { layer: '后端', items: ['Java 17', 'Spring Boot 3', 'MyBatis', 'JPA', 'Sa-Token', 'PageHelper'], note: '团队的单体服务统一提供内容接口' }, { layer: '数据与存储', items: ['MySQL', 'MinIO'], note: '内容数据与图片文件存储' }], gallery: [{ src: '/assets/v5/nanyang-1.png', step: 'STEP 01 / 读者端首页', caption: '读者端新闻首页（PC）。顶部是栏目导航与登录入口，中间为校庆主题轮播，下方是内容检索入口。这是面向师生的呈现层：截图里没有管理端的表格、增删改按钮或发布状态列。' }, { src: '/assets/v5/nanyang-2.png', step: 'STEP 02 / 栏目页', caption: '「学校概况 → 学校荣誉」栏目页。左侧是栏目导航，右侧是荣誉条目卡片。地址栏显示该页运行在本地开发端口 5175，属于本机实跑截图而非设计稿。' }, { src: '/assets/v5/nanyang-3.png', step: 'STEP 03 / 分类筛选', caption: '「校园快讯」分类筛选结果页。按分类与学院筛选后以卡片列表呈现新闻摘要，并支持重置筛选。同样是 PC 读者端页面，不是移动端或小程序。' }] },
  { id: 5, slug: 'ai-translator-vue-lab', title: '词流练习区', repoName: 'ai-translator-vue-lab', summary: '沿用词流的技术栈搭的练习场：把课程知识拆成章节，在真实项目里动手改造，而不是只记 API。', background: '练习场与正式产品共用同一套技术栈，减少重复搭建的成本；但两者边界清晰，练习产物不影响正式产品。', outcome: '把课程章节继续补完，并让每章的练习结果回流到词流。', role: '课程结构设计、练习环境搭建、章节内容编写', projectType: '练习场 / 技术试验', project_type: 'LAB', status: '持续练习中 · 内容未完整', tags: ['Vue 3', '课程章节', '练习环境'], image: '/assets/v5/lab-playground.png', visualKind: 'screenshot', evidence: '2 张真实运行截图 · 练习内容仍在补充中', stack: [{ layer: '前端', items: ['Vue 3', 'Element Plus', 'Pinia', 'Tailwind CSS'], note: '章节练习与可运行载体界面' }, { layer: '后端', items: ['FastAPI', 'SQLAlchemy', 'Alembic'], note: '与 LexiFlow 同源的服务骨架' }, { layer: '数据', items: ['PostgreSQL', 'Redis'], note: '沿用正式产品的数据选型' }, { layer: '运行', items: ['Docker Compose', 'Nginx'], note: '沿用正式产品的部署方式' }], gallery: [{ src: '/assets/v5/lab-playground.png', step: 'STEP 01 / 练习区', caption: '练习区首页。把学习拆成有目标的章节：先理解概念、运行示例、动手改造，再回到真实项目里找到它。当前已开课 8 个 Vue 实战章节。' }, { src: '/assets/v5/lab-home.png', step: 'STEP 02 / 可运行的载体', caption: '练习需要一个能跑起来的真实项目，而不是孤立的代码片段。练习区直接复用 LexiFlow 的词典与翻译页面作为实验载体。' }] },
]
