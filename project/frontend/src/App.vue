...
const fallbackProjects = [
  ...
  { id: 2, slug: 'ai-second-brain', title: 'AI Second Brain', summary: '从文档解析到 SSE 对话的个人知识库系统，让私有资料真正参与日常问答。', background: '把文档上传、切片、检索和流式对话连接起来。', outcome: '已完成独立项目闭环，正在整理公开版本证据。', role: '产品建模、Java 全栈开发、测试与部署', projectType: 'AI 产品 / 知识库 RAG', project_type: 'SOLO', status: '已实现 · 发布前复核', tags: ['RAG', 'Spring Boot', 'SSE'], image: '/assets/v5/brain-chat.png', visualKind: 'screenshot', evidence: '3 张已核验截图 · 新建会话发送首条消息的前端渲染缺陷待修', gallery: [{ src: '/assets/v5/secondbrain-login.png', step: 'STEP 01 / 身份入口', caption: '登录页。系统区分管理员与普通用户，文档、会话与模型配置按账号隔离，不共用同一份数据。' }, { src: '/assets/v5/brain-documents.png', step: 'STEP 02 / 文档管理', caption: '文档上传与处理状态。上传后经过解析、递归切片和本地 BGE 向量化，页面上显示每个文档的切片数量。截图为演示账号，未包含真实资料。' }, { src: '/assets/v5/brain-chat.png', step: 'STEP 03 / 核心链路', caption: '知识库对话。回答基于检索到的文档片段生成，并以 SSE 流式返回，下方可展开引用与相似度分数。' }] },
