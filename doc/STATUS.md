...
1. 重新部署 Netlify：`dist` 已重建、42 项验收已在生产产物上复跑通过，仅剩上传这一步（拖拽或 Netlify 面板）。
2. **详情页重构**（用户反馈"二级页面乱、文字扎堆、功能描述不明确"）。已定位的结构性原因：
   - 单张截图周围堆了 9 个元素，其中左右箭头、"点击放大"浮层与"← → 切换截图"提示**三处在解释同一操作**，页面在解释自己的 UI 而不是讲项目；
   - 侧栏卡片标题与内容不符：「问题与边界」渲染的是 `background`（技术链路描述），「下一步」渲染的是 `outcome`（实际是状态）；
   - 同一字段在不同案例里语义不一致：Folio 的 `outcome` 是"已完成…"，知衡的 `outcome` 是真正的下一步；
   - 前提信息（问题、边界）被放进侧栏小卡片，而 hero 右栏只放「状态 / 角色 / 仓库名」三行纯文本；
   - 全站没有用上任何硬数字（如知衡的「8 次迁移 · 51 项后端测试」、本项目的 42 项验收）。
   改法：图组 9→4 个元素；正文改「问题 → 做法 → 结果」三段式；修正卡片标题或内容；首屏补入事实数字；图注压到 1–2 句且只讲该图证明了什么。
3. Gitee 仓库改名：`my-translate` → `ai-translator`、`lexi-flow-practice` → `ai-translator-vue-lab`，
   改名后更新本地 remote。
4. 轮换 LexiFlow 已公开的 Postgres 密码，并把 `docker-compose.yml` 改为环境变量注入。
5. Folio 文档页明暗混杂（深色外壳 + 纯白上传区与文档卡片）：应**改 Folio 前端主题后重新截图**，
   不得给现有截图上色。
6. 确认 LexiFlow 练习区仓库的定位后再决定是否推送（当前它包含整个 LexiFlow 代码库）。
7. 知衡考试分析页返回 500（工作台、题库、改卷三页正常），需定位服务端异常。
8. 补数据库接口读写、事务失败回滚、测试数据库隔离验证（仅在解冻后端时才有意义）。
