...
**当前推进：阶段 0 门禁 · 任务 0.4（部署自动化）——路线见 `doc/23-roadmap.md`。**
统一门禁命令（`node utils/verify.mjs`）与推送钩子已就位并完成双向验证：注入假凭据被拦下，
含后端改动的推送会真的跑后端测试。钩子随仓库走，clone 后跑一次 `node utils/setup.mjs` 即装好。
