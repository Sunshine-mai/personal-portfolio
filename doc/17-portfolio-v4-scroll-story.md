...
V4 在 V3 的作品优先信息架构上增加滚动叙事：首屏直接展示两件精选作品，访客向下滚动时依次打开首页、AI 作品、剪辑作品、灵感与计划、关于我五个章节。页面仍然以可扫描的作品档案为主体，动画只负责揭示层级，不接管用户滚轮。新版正文采用更大的阅读字号，减少说明文案对作品的遮挡。
...
- `IntersectionObserver` 在章节进入视口后加入可见状态，依次揭示标题、筛选和章节脚注；作品卡片在各自进入视口时单独揭示。
...
- 2026-09-06 浏览器自动化验收：抽屉、筛选逻辑、收藏持久化、邮箱复制反馈、移动端菜单通过；发现 AI 章节因观察器 `threshold: .35` 高于该章节可达的最大可见比例（章节高 2063px，视口 720px），导致章节标题、筛选栏和脚注不显示，且导航高亮停留在中途章节。已修复：揭示改为 `threshold: .12` 一次性触发，导航高亮改为按滚动位置实时计算；补充作品卡片鼠标点击打开详情抽屉，与本文档交互描述一致。修复后回归通过：AI 章节标题/筛选可见，导航随滚动位置正确切换，卡片点击可开抽屉。ESC 与键盘打开路径受自动化环境按键派发限制，需人工抽查一次。
