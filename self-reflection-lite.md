...
| 宽度要加在需要它的元素上，不是它的祖先上 | 想"让技术网络图更宽"，我先加宽了整节的内容栏 → **标题脱离全站版心**；再改成加宽整个组件 → **图下的文字跟着跑到宽边界**。第三次才收窄到只剩 SVG。一轮内犯了三次同类错误：**意图对，但改在了错误的层级。** |
| 断言必须走用户实际走的那条路 | 我给锚点加了 `[id]{scroll-margin-top:88px}` 并写了断言，断言通过。但所有者点导航时标题仍被吸顶栏盖住——因为我的断言用 `scrollIntoView`（**遵守** scroll-margin），而他走 Vue Router 的 `scrollBehavior`（**不读** CSS）。**测了另一条路径等于没测**，还制造了"已修复"的假信心。改后断言改为逐条点击五个导航链接。 |
| 断言只查下限会放过"太过" | 编号落位断言写的是 `top >= 80`，于是编号出现在 160px（空隙大得刺眼）时**照样通过**。所有者反馈"空隙也太多了"才发现。**只挡"不够"、不挡"过头"的断言，等于只做了一半。** 现在写成 80~120 区间。 |
| 物理限制要写进断言，不要排除目标 | 「关于我」是最后一节，页面已到底、无法把标题顶到 96px。我没有把该节从断言里删掉（那会掩盖真问题），而是把真实要求写成"**要么落位合适、要么页面已到底**"。 |
| 改对外展示名必须连截图一起改 | 想把 LexiFlow / Folio 改成中文名，但两个应用界面与**所有截图**里都印着旧名（页头组件每页都渲染）。只改作品集文字就会重演案例四那个"图文不符"。最终采用"**中文名在前、英文名保留**"：一眼知道是什么，同时截图仍然对得上，不必重新采集。**改名不是改字符串，是改一整套证据。** |
